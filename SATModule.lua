...
local function enforceType( object: any )
	...
	elseif object.Type == 'polygon' then
		...
		for _, vertex in ipairs( object.Vertices ) do
			assert( typeof( vertex ) == 'Vector2', 'Vertices contains invalid elements. Should be an array of Vector2 elements.' )
...
local function getPolygonNormals( polygon: Polygon )
	local normals = {}
...
local function filterUniqueNormals( normals: { Vector2 } )
	local uniqueNormals = {}
...
local function projectVertices( vertices: { Vector2 }, axis: Vector2 )
...
local function projectObject( object: any, axis: Vector2 )
	local vertices
...
local function checkNumberBetweenNumbers( num: number, min: number, max: number )
	return num >= min and num <= max
end

local function checkForOverlap( minA: number, maxA: number, minB: number, maxB: number )
...
function SATModule:CreatePoint( position: Vector2 )
...
function SATModule:CreateCircle( position: Vector2, radius: number )
	assert( position, 'Argument 1 (Position) missing.' )
	assert( typeof( position ) == 'Vector2', 'Argument 1 (Position) invalid. Should be a Vector2.' )
	assert( radius, 'Argument 2 (Radius) missing.' )
	assert( typeof( radius ) == 'number' and radius > 0, 'Argument 2 (Radius) invalid. Should be a positive number.' )

	local circle: Circle = { Type = 'circle', Position = position, Radius = radius }
	return circle
end

function SATModule:CreatePolygon( vertices: { Vector2 } )
	assert( vertices, 'Argument 1 (Vertices) missing.' )
	local array = {}
	for _, vertex in ipairs( vertices ) do
		assert( typeof( vertex ) == 'Vector2', 'Argument 1 (Vertices) contains invalid elements. Should be an array of Vector2 elements.' )
		table.insert( array, vertex )
	end
...
function SATModule:RotatePolygon( polygon: Polygon, radians: number, centre: Vector2 )
...
function SATModule:AreObjectsColliding( objectA: any, objectB: any )
	...
	if ( objectA.Type == 'circle' or objectB.Type == 'circle' ) and ( objectA.Type == 'point' or objectB.Type == 'point' ) then
